...
namespace Oxide.Plugins
{
    ...
    public class AutoElevators : RustPlugin
    {
        ...
        void Unload()
        {
            if (ElevatorUp != null)
            {
                foreach (var entity in BaseNetworkable.serverEntities.OfType<Elevator>())
                {
                    Elevator x = entity;
                    if (x != null)
                    {
            ...
            object OnElevatorMove(Elevator e)
            {
                ...
                if (ThisElevator != null)
                {
                    ...
                    {
                        GoToFloor(ThisElevator.Floors, e);
                        if (ThisElevator.AutoReturn)
                        {
                            timer.Once(ThisElevator.ReturnTime, () =>
                             {
                                 if (Up.Contains(e.net.ID))
                                 {
                                     GoToFloor(1, e);
            ...
            void GoToFloor(int floor, Elevator e)
            {
